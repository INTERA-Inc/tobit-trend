...
def test_chemistry_import_applies_duplicate_priority_after_filters(tmp_path):
    chem = pd.DataFrame(
        {
            "NAME": ["W1", "W1"],
            "EVENT": pd.to_datetime(["2024-01-01", "2024-01-01"]),
            "ANALYTE": ["Analyte", "Analyte"],
            "ANALYTE_ORG": ["Chromium", "Hexavalent Chromium"],
            "DUPLICATE_PRIORITY": [0, 1],
            "FILTERED": ["Y", "N"],
            "VAL": [10.0, 5.0],
            "LABQ": ["", ""],
            "UNIT": ["ug/L", "ug/L"],
    ...
    obs = out.loc[out["VAL"].notna()]

    assert len(obs) == 1
    assert obs["VAL"].iloc[0] == 5.0
