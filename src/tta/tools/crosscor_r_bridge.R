#!/usr/bin/env Rscript
#
# Called once per well/TERM with all lag values in a single CSV.
# Columns: x1, y1 (chemistry series), x2, y2 (stage series), lag (lag vector).
# Series and lag vector may differ in length; NAs pad the shorter column(s).
# Returns one row per lag: columns acf, lag.
...
lags <- sort(unique(dat$lag[!is.na(dat$lag)]))
...
results <- lapply(lags, function(lag) crosscor_exact(x1, y1, x2, y2, lag))
